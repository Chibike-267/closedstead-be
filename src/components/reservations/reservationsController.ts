...
export const updateReservation = async (req: Request, res: Response) => {
    try {
      ...
      const reservation = await ReservationsModel.findOne({ where: { id, userId } });
  ...
  export const reservationBelongingToUser = async (req: Request, res: Response) => {
    try {
  ...
  export const reservationBelongingToUnit = async (req: Request, res: Response) => {
    try {
      ...
      if (!unitId) {
        return res.status(400).json({ message: "unitId is required in the request parameters or body" });
      }
  
      const reservations = await ReservationsModel.findAll({ where: { unitId } });
  
      if (!reservations || reservations.length === 0) {
        return res.status(404).json({ message: "Reservations not found for the unit" });
      }
