...
export const cancell = async (req: UserRequest, res: Response) => {
  try {
    ...
    if (reservation.status !== "cancelled" && reservation.status !== "stayed" ) {
      await ReservationsModel.update(
        { status: "cancelled" },
        { where: { id } }
      );
    ...
    return res.status(400).json({ message: "Reservation has already been cancelled or you need to be cheched out to cancell" });
  }
}catch (error: any) {
    console.error("Error Checking out Client:", error);
    return res.status(500).json({ error: error.message || "Something went wrong" });
